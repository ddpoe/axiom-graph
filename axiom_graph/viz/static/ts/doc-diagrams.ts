...
// Mermaid is loaded via CDN <script> tag (not ESM)
declare const mermaid: any;

// ── State ───────────────────────────────────────────────────────────────────

let _diagramEditorIdx: number | null = null;
let _diagramMonaco: any = null;
let _diagramPreviewTimer: ReturnType<typeof setTimeout> | null = null;
let _mermaidMonacoRegistered = false;
let _mermaidSources: string[] = [];
...
export function isMermaidSection(content: string): boolean {
  if (!content) return false;
  return /```mermaid/i.test(content);
}

export function extractMermaidSource(content: string): string {
  if (!content) return '';
...
export async function runMermaid(): Promise<void> {
  ...
  document.querySelectorAll('.doc-section-content .mermaid[data-mermaid-idx]').forEach(el => {
    const idx = parseInt((el as HTMLElement).dataset.mermaidIdx || '-1', 10);
  ...
  document.querySelectorAll('.doc-section--diagram .doc-mermaid-block .mermaid').forEach(pre => {
    ...
    (pre as HTMLElement).style.removeProperty('width');
    (pre as HTMLElement).style.removeProperty('max-width');
  });
}

// ── Diagram editor lifecycle ────────────────────────────────────────────────

export function openDiagramEditor(idx: number): void {
  _diagramEditorIdx = idx;
}

export function closeDiagramEditor(): void {
  _diagramEditorIdx = null;
  destroyDiagramEditor();
}

export function applyDiagramEditor(
  idx: number,
  renderedSections: any[],
  rawSections: any[],
  setDirty: () => void,
): void {
  if (!_diagramMonaco) return;
...
export function destroyDiagramEditor(): void {
...
export function mountDiagramEditor(idx: number, source: string): void {
  ...
  _diagramMonaco.onDidChangeModelContent(() => {
    if (_diagramPreviewTimer) clearTimeout(_diagramPreviewTimer);
...
async function updateDiagramPreview(container: HTMLElement, source: string): Promise<void> {
...
function registerMermaidLanguage(): void {
  if (_mermaidMonacoRegistered) return;
  const m = (window as any).monaco;
  if (!m) return;
