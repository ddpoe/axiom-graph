...
/** One selectable workflow in the picker. */
export interface ExportPickerItem {
  id: string;
  name: string;
  module: string;
}
...
export function openExportPicker(items: ExportPickerItem[]): void {
  _items = items;
  _selected = new Set();
  _search = '';
  _bindOnce();
  ...
  if (!overlay) return;
  const searchInput = document.getElementById('wf-export-search') as HTMLInputElement | null;
...
export function closeExportPicker(): void {
  const overlay = _overlay();
...
function _bindOnce(): void {
  ...
  const overlay = _overlay();
  if (overlay) {
    overlay.addEventListener('click', (e) => { if (e.target === overlay) closeExportPicker(); });
  }
  ...
  if (selectAll) {
    selectAll.addEventListener('click', () => {
      ...
      for (const item of visible) {
        if (allOn) _selected.delete(item.id);
  ...
  if (runBtn) runBtn.addEventListener('click', () => _runExport());

  document.addEventListener('keydown', (e) => {
    if (e.key !== 'Escape') return;
    const el = _overlay();
    if (el && !el.classList.contains('hidden')) closeExportPicker();
  });
    ...
    listEl.addEventListener('change', (e) => {
      ...
      if (moduleKey !== undefined) {
        for (const item of _visible().filter(i => i.module === moduleKey)) {
          if (cb.checked) _selected.add(item.id);
          else _selected.delete(item.id);
        }
        _render();
        return;
      }
      ...
      if (!wfId) return;
      if (cb.checked) _selected.add(wfId);
      else _selected.delete(wfId);
      _updateCount();
    });
  }
}

function _visible(): ExportPickerItem[] {
  if (!_search) return _items;
  return _items.filter(i =>
    i.name.toLowerCase().includes(_search) || i.module.toLowerCase().includes(_search));
}
...
function _render(): void {
  ...
  const groups = new Map<string, ExportPickerItem[]>();
