...
import type { RawDoc, DocSection, RenderedDoc } from './types.js';
import { esc, apiFetch } from './view-utils.js';
import {
  getRawDoc, setRawDoc, isDirty, setDirty, getSelectedDocId, saveDoc,
  getPendingRenames, clearPendingRenames, addPendingRename,
  destroyRawMonaco, setRawMonaco, getRawMonaco,
} from './doc-api.js';
import {
...
let _renderedData: RenderedDoc | null = null;
let _editingSectionIdx: number | null = null;
let _editingHeadingIdx: number | null = null;
let _editingSlugIdx: number | null = null;
let _linkSearchIdx: number | null = null;
let _linkSearchResults: any[] = [];
let _linkSearchTimer: ReturnType<typeof setTimeout> | null = null;
let _tiptapEditors: Record<string, any> = {};
let _viewRaw = false;
...
export function getRenderedData(): RenderedDoc | null { return _renderedData; }
export function setRenderedData(data: RenderedDoc | null): void { _renderedData = data; }
export function isViewRaw(): boolean { return _viewRaw; }
export function setViewRaw(raw: boolean): void { _viewRaw = raw; }
...
function getCollapsedSections(): Set<string> {
  if (!_collapsedSections) {
    ...
    } catch { _collapsedSections = new Set(); }
  }
...
function getExpandedSubsections(): Set<string> {
  if (!_expandedSubsections) {
    ...
    } catch { _expandedSubsections = new Set(); }
  }
  return _expandedSubsections;
}

function persistExpandedSubsections(): void {
  sessionStorage.setItem('cortex-doc-expanded-subs', JSON.stringify([...getExpandedSubsections()]));
}

function toggleSubsectionExpand(sectionId: string): void {
...
export function localSlug(fullId: string): string {
  if (!fullId) return '';
...
function replaceLocalSlug(fullId: string, newLocal: string): string {
  if (!fullId || !fullId.includes('::')) return newLocal;
...
function slugify(text: string, maxWords = 5, maxLen = 40): string {
  let slug = text
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '');
  const words = slug.split('-').filter(Boolean);
  if (words.length > maxWords) slug = words.slice(0, maxWords).join('-');
...
function flattenRawSections(sections: any[]): any[] {
  const result: any[] = [];
  for (const sec of sections) {
    const { sections: children, ...rest } = sec;
    result.push(rest);
    if (children && children.length) result.push(...flattenRawSections(children));
  }
  return result;
}
 ...
 */
function nestRawSections(flatSections: any[]): any[] {
  const rendered = _renderedData?.sections || [];
  const root: any[] = [];
  const stack: Array<{ target: any[]; depth: number }> = [{ target: root, depth: -1 }];
  ...
  const clean = (secs: any[]) => {
    for (const s of secs) {
      if (s.sections.length === 0) delete s.sections;
      else clean(s.sections);
    }
  };
  clean(root);
  return root;
...
function isAncestorCollapsed(sections: DocSection[], idx: number): boolean {
  const collapsed = getCollapsedSections();
  let sec = sections[idx];
  while (sec && sec.parent_id) {
    if (collapsed.has(sec.parent_id)) return true;
    sec = sections.find(s => s.id === sec.parent_id) as DocSection;
  }
  return false;
}

function sectionHasChildren(sections: DocSection[], idx: number): boolean {
...
function _buildNestedList(block: string, ordered: boolean): string {
...
export function renderMarkdown(text: string): string {
  ...
  html = html.replace(/((?:^\|.+\|$\n?)+)/gm, (tableBlock) => {
    ...
    rows.forEach((row, i) => {
      ...
      const cells = row.split('|').filter((_: string, ci: number) => ci > 0 && ci < row.split('|').length - 1);
      const tag = i === 0 ? 'th' : 'td';
      if (i === 0) tableHtml += '<thead>';
      if (i === 2 || (i === 1 && !/^\|[\s\-:|]+\|$/.test(rows[1]))) tableHtml += '<tbody>';
      tableHtml += '<tr>';
      cells.forEach(cell => {
...
export function mountTiptapEditors(): void {
  if (!(window as any)._tiptapReady) {
    console.warn('[docs] Tiptap not ready yet');
    return;
  }

  document.querySelectorAll('.doc-tiptap-editor[data-section-idx]').forEach(el => {
    const idx = (el as HTMLElement).dataset.sectionIdx || '';
    if (_tiptapEditors[idx]) return;

    const extensions = [(window as any).TiptapStarterKit];
    if ((window as any).TiptapTable) extensions.push((window as any).TiptapTable.configure({ resizable: false }));
    if ((window as any).TiptapTableRow) extensions.push((window as any).TiptapTableRow);
    if ((window as any).TiptapTableCell) extensions.push((window as any).TiptapTableCell);
    if ((window as any).TiptapTableHeader) extensions.push((window as any).TiptapTableHeader);
    ...
    const editor = new (window as any).TiptapEditor({
      element: el,
      extensions,
      content: htmlContent,
      editorProps: {
    ...
    if (toolbar && toolbar.classList.contains('doc-tiptap-toolbar')) {
      editor.on('transaction', () => {
        toolbar.querySelectorAll('.tt-btn').forEach((tbBtn: Element) => {
          const cmd = (tbBtn as HTMLElement).dataset.cmd;
          let isActive = false;
          switch (cmd) {
            ...
            case 'heading':    isActive = editor.isActive('heading', { level: parseInt((tbBtn as HTMLElement).dataset.level || '3') }); break;
  ...
  document.querySelectorAll('.doc-tiptap-toolbar').forEach(toolbar => {
    toolbar.querySelectorAll('button[data-cmd]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const idx = (toolbar as HTMLElement).dataset.sectionIdx || '';
        ...
        switch (cmd) {
          ...
          case 'heading': chain.toggleHeading({ level: parseInt((btn as HTMLElement).dataset.level || '3') }).run(); break;
...
export function destroyEditors(flush = true): void {
  if (flush) flushEditors();
...
function destroyScrollObserver(): void {
...
export function addSection(): void {
  const rawDoc = getRawDoc();
  if (!rawDoc) return;
  if (!rawDoc.sections) rawDoc.sections = [];
...
export function addSubSection(parentIdx: number): void {
  const rawDoc = getRawDoc();
  if (!rawDoc || !rawDoc.sections) return;
...
export function moveSection(idx: number, dir: number): void {
  if (!_renderedData || !_renderedData.sections) return;
  const targetIdx = idx + dir;
  if (targetIdx < 0 || targetIdx >= _renderedData.sections.length) return;
  ...
  if (_editingSectionIdx === idx) _editingSectionIdx = targetIdx;
  else if (_editingSectionIdx === targetIdx) _editingSectionIdx = idx;
  if (_editingHeadingIdx === idx) _editingHeadingIdx = targetIdx;
  else if (_editingHeadingIdx === targetIdx) _editingHeadingIdx = idx;
  if (_editingSlugIdx === idx) _editingSlugIdx = targetIdx;
  else if (_editingSlugIdx === targetIdx) _editingSlugIdx = idx;
  if (_linkSearchIdx === idx) _linkSearchIdx = targetIdx;
  else if (_linkSearchIdx === targetIdx) _linkSearchIdx = idx;
}

export function removeSection(idx: number): void {
  const rawDoc = getRawDoc();
  if (rawDoc && rawDoc.sections) {
  ...
  setDirty(true);
  if (_editingSectionIdx === idx) _editingSectionIdx = null;
  if (_editingHeadingIdx === idx) _editingHeadingIdx = null;
  if (_editingSlugIdx === idx) _editingSlugIdx = null;
  if (_linkSearchIdx === idx) _linkSearchIdx = null;
}

// ── Tag management ──────────────────────────────────────────────────────────

export function addDocTag(tag: string): void {
  const rawDoc = getRawDoc();
  if (!rawDoc) return;
  if (!rawDoc.tags) rawDoc.tags = [];
  if (rawDoc.tags.includes(tag)) return;
  rawDoc.tags.push(tag);
  setDirty(true);
}

export function removeDocTag(tagIdx: number): void {
  const rawDoc = getRawDoc();
  if (!rawDoc || !rawDoc.tags) return;
  rawDoc.tags.splice(tagIdx, 1);
  setDirty(true);
}

export function addSectionTag(secIdx: number, tag: string): void {
  const rawDoc = getRawDoc();
  if (!rawDoc || !rawDoc.sections || !rawDoc.sections[secIdx]) return;
  const sec = rawDoc.sections[secIdx];
  if (!sec.tags) sec.tags = [];
  if (sec.tags.includes(tag)) return;
  sec.tags.push(tag);
  if (_renderedData && _renderedData.sections && _renderedData.sections[secIdx]) {
    if (!_renderedData.sections[secIdx].tags) _renderedData.sections[secIdx].tags = [];
    _renderedData.sections[secIdx].tags!.push(tag);
  }
  setDirty(true);
}

export function removeSectionTag(secIdx: number, tagIdx: number): void {
  const rawDoc = getRawDoc();
  if (!rawDoc || !rawDoc.sections || !rawDoc.sections[secIdx]) return;
  const sec = rawDoc.sections[secIdx];
  if (!sec.tags) return;
...
export async function doLinkSearch(query: string, sectionIdx: number): Promise<void> {
  if (!query) {
    _linkSearchResults = [];
    return;
  }
...
export function getLinkSearchResults(): any[] { return _linkSearchResults; }

export function addLink(sectionIdx: number, nodeId: string): void {
  const rawDoc = getRawDoc();
  if (!rawDoc || !rawDoc.sections || !rawDoc.sections[sectionIdx]) return;
  const sec = rawDoc.sections[sectionIdx];
  if (!sec.links) sec.links = [];
  if (sec.links.some((l: any) => l.node_id === nodeId)) return;
  ...
  if (_renderedData && _renderedData.sections && _renderedData.sections[sectionIdx]) {
    ...
    if (!renderedSec.linked_nodes) renderedSec.linked_nodes = [];
...
export function removeLink(sectionIdx: number, linkIdx: number): void {
  const rawDoc = getRawDoc();
  if (!rawDoc || !rawDoc.sections || !rawDoc.sections[sectionIdx]) return;
  const sec = rawDoc.sections[sectionIdx];
  if (!sec.links) return;
  ...
  if (_renderedData && _renderedData.sections && _renderedData.sections[sectionIdx]) {
    ...
    if (renderedSec.linked_nodes && removedNodeId) {
      ...
      if (rIdx >= 0) renderedSec.linked_nodes.splice(rIdx, 1);
    }
  }
  setDirty(true);
}
...
export async function save(): Promise<boolean> {
  ...
  const slugsByParent: Record<string, Array<{ id: string; idx: number }>> = {};
  sections.forEach((sec, idx) => {
    const key = sec.parent_id || '__root__';
    if (!slugsByParent[key]) slugsByParent[key] = [];
  ...
  if (errors.length > 0) {
    ...
    errors.forEach(e => {
      ...
      if (secEl) {
        ...
        let errDiv = secEl.querySelector('.doc-section-error') as HTMLElement | null;
        if (!errDiv) {
          errDiv = document.createElement('div');
          errDiv.className = 'doc-section-error';
          const header = secEl.querySelector('.doc-section-header');
          if (header) header.after(errDiv);
        }
        errDiv.textContent = '\u26A0 ' + e.msg;
      }
    });
    const saveBtn = document.getElementById('doc-save-btn');
    if (saveBtn) saveBtn.textContent = `Save blocked \u2014 ${errors.length} issue${errors.length > 1 ? 's' : ''}`;
    setTimeout(() => { const btn = document.getElementById('doc-save-btn'); if (btn) btn.textContent = 'Save'; }, 3000);
    return false;
  }
  ...
  try {
    ...
    await saveDoc(docId, docToSave as RawDoc, getPendingRenames());
    clearPendingRenames();
    ...
    _renderedData = renderData as RenderedDoc;
    const fetchedDoc = rawData.doc;
    if (fetchedDoc && fetchedDoc.sections) {
      setRawDoc({ ...fetchedDoc, sections: flattenRawSections(fetchedDoc.sections as any[]) } as RawDoc);
    } else {
    ...
    setTimeout(() => { const btn = document.getElementById('doc-save-btn'); if (btn) btn.textContent = 'Save'; }, 2000);
    return true;
  } catch (err: any) {
    if (saveBtn) {
...
export function renderToc(sections: DocSection[]): string {
 ...
 * the JSON is invalid.
 */
async function toggleRawSource(): Promise<void> {
  if (_viewRaw) {
    ...
    if (rawMonaco) {
      ...
      const textarea = document.getElementById('doc-raw-source-textarea') as HTMLTextAreaElement | null;
    ...
    if (changed) {
      ...
      try {
        ...
        if (errorEl) { errorEl.textContent = 'Invalid JSON: ' + e.message; errorEl.style.display = 'block'; }
        return; // stay in raw view
      }
      if (typeof parsed !== 'object' || Array.isArray(parsed)) {
        if (errorEl) { errorEl.textContent = 'JSON must be an object (not an array)'; errorEl.style.display = 'block'; }
        return;
      }
      if (!parsed.title || !String(parsed.title).trim()) {
        if (errorEl) { errorEl.textContent = 'JSON must have a non-empty "title" field'; errorEl.style.display = 'block'; }
        return;
      }
      ...
      try {
        ...
        if (docId) {
          ...
          if (rawData.doc && rawData.doc.sections) {
            setRawDoc({ ...rawData.doc, sections: flattenRawSections(rawData.doc.sections as any[]) } as RawDoc);
          }
        }
      } catch (err: any) {
        if (errorEl) { errorEl.textContent = 'Save failed: ' + err.message; errorEl.style.display = 'block'; }
        return;
      }
    }
 ...
 */
export function renderDocContent(contentEl: HTMLElement): void {
  if (!_renderedData) return;

  // Reset mermaid source collector
  clearMermaidSources();

  const { title, sections } = _renderedData;
  const rawDoc = getRawDoc() || ({} as any);
    ...
    <div class="doc-content-inner${isWide() ? ' wide' : ''}">
      ...
      <div class="doc-tags-bar" id="doc-tags-bar">
        ...
        <div class="doc-tags-chips" id="doc-tags-chips">
          ${docTags.map((t: string, ti: number) => `
    ...
    sections.forEach((sec: any, idx: number) => {
      ...
      const tagsHtml = (): string => `
        <div class="doc-section-tags-bar" data-idx="${idx}">
          ...
          ${secTags.map((t: string, ti: number) => `
      ...
      const linksHtml = (): string => {
        ...
        secLinks.forEach((lnk: any, li: number) => {
          const display = displayLinks.find((d: any) => d.node_id === lnk.node_id);
          const label = display
      ...
      if (secDepth >= 1) {
        ...
        for (let j = idx + 1; j < sections.length && (sections[j].depth || 0) > secDepth; j++) {
          if ((sections[j].depth || 0) === secDepth + 1) childCount++;
        }
        ...
        if (isSubExpanded) {
          ...
          secTags.forEach((t: string, ti: number) => {
...
function _renderLinkSearchResults(sectionIdx: number, existingLinks: any[]): string {
  const existingIds = new Set(existingLinks.map((l: any) => l.node_id));
...
function _renderTocSidebar(contentEl: HTMLElement): void {
  if (!_renderedData || !_renderedData.sections) return;
  destroyScrollObserver();

  const scrollRoot = contentEl.querySelector('#doc-content') as HTMLElement | null;
    ...
    for (const entry of entries) {
      if (entry.isIntersecting) {
        ...
        document.querySelectorAll('.doc-toc-item').forEach(el => {
          el.classList.toggle('active', (el as HTMLElement).dataset.sectionIdx === idx);
        });
      }
    }
  }, {
...
function _wireRawSourceEvents(contentEl: HTMLElement): void {
  const textarea = contentEl.querySelector('#doc-raw-source-textarea') as HTMLTextAreaElement | null;
  if (textarea) {
    textarea.addEventListener('input', () => { setDirty(true); });
  }
}

function _mountRawMonaco(): void {
 ...
 */
export function wireDocEvents(contentEl: HTMLElement): void {
  ...
  contentEl.querySelectorAll('.doc-diagram-expand-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      (btn as HTMLElement).classList.toggle('active', isExpanded);
      (btn as HTMLElement).innerHTML = isExpanded ? '&#x2921; Collapse' : '&#x2922; Expand';
    });
  });
  ...
  if (wideBtn) {
    wideBtn.addEventListener('click', () => {
      ...
      if (inner) inner.classList.toggle('wide', goWide);
      (wideBtn as HTMLElement).classList.toggle('active', goWide);
  ...
  contentEl.querySelectorAll('.doc-section-content').forEach(el => {
    ...
    el.addEventListener('dblclick', () => {
      const idx = parseInt((section as HTMLElement).dataset.idx || '0', 10);
  ...
  contentEl.querySelectorAll('.doc-section-heading').forEach(el => {
    el.addEventListener('click', () => {
      ...
      requestAnimationFrame(() => {
        const inp = document.querySelector(`.doc-heading-input[data-idx="${idx}"]`) as HTMLInputElement | null;
  ...
  contentEl.querySelectorAll('.doc-heading-save-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const idx = parseInt((btn as HTMLElement).dataset.idx || '0', 10);
      const inp = contentEl.querySelector(`.doc-heading-input[data-idx="${idx}"]`) as HTMLInputElement | null;
      if (!inp) return;
  ...
  contentEl.querySelectorAll('.doc-heading-input').forEach(inp => {
    inp.addEventListener('keydown', (e: Event) => {
      ...
      if (ke.key === 'Enter') {
        ke.preventDefault();
        const idx = parseInt((inp as HTMLElement).dataset.idx || '0', 10);
        (contentEl.querySelector(`.doc-heading-save-btn[data-idx="${idx}"]`) as HTMLElement)?.click();
  ...
  contentEl.querySelectorAll('.doc-slug-text').forEach(el => {
    el.addEventListener('click', () => {
      ...
      _rerender();
      requestAnimationFrame(() => {
        const inp = document.querySelector(`.doc-slug-input[data-idx="${idx}"]`) as HTMLInputElement | null;
        if (inp) { inp.focus(); inp.select(); }
      });
    });
  });
  ...
  contentEl.querySelectorAll('.doc-slug-save-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const idx = parseInt((btn as HTMLElement).dataset.idx || '0', 10);
      const inp = contentEl.querySelector(`.doc-slug-input[data-idx="${idx}"]`) as HTMLInputElement | null;
      if (!inp) return;
      ...
      const duplicate = sections.some((s: any, i: number) => i !== idx && localSlug(s.id) === newLocal && s.parent_id === currentSec.parent_id);
      if (duplicate) {
        const conflictEl = contentEl.querySelector(`.doc-slug-conflict[data-idx="${idx}"]`) as HTMLElement;
        if (conflictEl) {
  ...
  contentEl.querySelectorAll('.doc-slug-input').forEach(inp => {
    inp.addEventListener('input', () => {
      const idx = parseInt((inp as HTMLElement).dataset.idx || '0', 10);
      const testSlug = slugify((inp as HTMLInputElement).value);
      const sections = _renderedData?.sections;
      if (!sections) return;
      const currentSec = sections[idx];
      const duplicate = sections.some((s: any, i: number) => i !== idx && localSlug(s.id) === testSlug && s.parent_id === currentSec.parent_id);
      const conflictEl = contentEl.querySelector(`.doc-slug-conflict[data-idx="${idx}"]`) as HTMLElement;
      if (conflictEl) {
        if (duplicate) {
    ...
    inp.addEventListener('keydown', (e: Event) => {
      ...
      if (ke.key === 'Enter') {
        ke.preventDefault();
        const idx = parseInt((inp as HTMLElement).dataset.idx || '0', 10);
        (contentEl.querySelector(`.doc-slug-save-btn[data-idx="${idx}"]`) as HTMLElement)?.click();
      } else if (ke.key === 'Escape') {
        _editingSlugIdx = null;
        _rerender();
      }
  ...
  contentEl.querySelectorAll('.doc-move-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const idx = parseInt((btn as HTMLElement).dataset.idx || '0', 10);
      const dir = (btn as HTMLElement).dataset.dir === 'up' ? -1 : 1;
      moveSection(idx, dir);
      _rerender();
    });
  });
  ...
  contentEl.querySelectorAll('.doc-remove-section-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (!confirm(`Remove section "${heading}"? This can be undone by not saving.`)) return;
      removeSection(idx);
      _rerender();
    });
  });
  ...
  contentEl.querySelectorAll('.doc-collapse-toggle').forEach(btn => {
    btn.addEventListener('click', (e: Event) => {
      e.stopPropagation();
      toggleSectionCollapse((btn as HTMLElement).dataset.sectionId || '');
      _rerender();
    });
  });
  ...
  contentEl.querySelectorAll('.sub-row-header').forEach(el => {
    el.addEventListener('click', (e: Event) => {
      ...
      if ((e.target as HTMLElement).closest('.sub-header-actions')) return;
      toggleSubsectionExpand((el as HTMLElement).dataset.sectionId || '');
      _rerender();
    });
  });
  ...
  contentEl.querySelectorAll('.sub-tag-add').forEach(el => {
    el.addEventListener('click', (e: Event) => {
      ...
      const commit = () => {
        ...
        if (tag) { addSectionTag(idx, tag); }
        _rerender();
      };
      input.addEventListener('keydown', (ke: KeyboardEvent) => {
        if (ke.key === 'Enter') { ke.preventDefault(); commit(); }
        if (ke.key === 'Escape') { _rerender(); }
      });
  ...
  contentEl.querySelectorAll('.sub-link-add').forEach(el => {
    el.addEventListener('click', (e: Event) => {
      ...
      if (_linkSearchIdx !== null) {
        requestAnimationFrame(() => {
          const inp = contentEl.querySelector(`.doc-link-search-input[data-idx="${idx}"]`) as HTMLInputElement | null;
          if (inp) inp.focus();
        });
      }
    });
  ...
  contentEl.querySelectorAll('.doc-add-subsection-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      addSubSection(parseInt((btn as HTMLElement).dataset.idx || '0', 10));
      _rerender();
    });
  });
  ...
  contentEl.querySelectorAll('.doc-tag-remove').forEach(btn => {
    btn.addEventListener('click', (e: Event) => {
      e.stopPropagation();
      const scope = (btn as HTMLElement).dataset.scope;
      const tagIdx = parseInt((btn as HTMLElement).dataset.tagIdx || '0', 10);
      if (scope === 'doc') {
        removeDocTag(tagIdx);
      } else if (scope === 'section') {
        const secIdx = parseInt((btn as HTMLElement).dataset.sectionIdx || '0', 10);
  ...
  // Doc-level tag add
  const docTagInput = contentEl.querySelector('#doc-tag-add-input') as HTMLInputElement | null;
  if (docTagInput) {
    docTagInput.addEventListener('keydown', (e: Event) => {
      const ke = e as KeyboardEvent;
      if (ke.key === 'Enter') {
        ke.preventDefault();
        const tag = docTagInput.value.trim();
        if (tag) { addDocTag(tag); _rerender(); }
      }
    });
  }
  ...
  contentEl.querySelectorAll('.doc-stag-add-input').forEach(inp => {
    inp.addEventListener('keydown', (e: Event) => {
      ...
      if (ke.key === 'Enter') {
        ke.preventDefault();
        const idx = parseInt((inp as HTMLElement).dataset.idx || '0', 10);
        const tag = (inp as HTMLInputElement).value.trim();
        if (tag) { addSectionTag(idx, tag); _rerender(); }
      }
    });
  });
  ...
  contentEl.querySelectorAll('.doc-link-search-input').forEach(inp => {
    inp.addEventListener('input', () => {
      const idx = parseInt((inp as HTMLElement).dataset.idx || '0', 10);
      const query = (inp as HTMLInputElement).value.trim();
  ...
  contentEl.querySelectorAll('.doc-link-search-item:not(.already-linked)').forEach(item => {
    item.addEventListener('click', () => {
      const secIdx = parseInt((item as HTMLElement).dataset.sectionIdx || '0', 10);
      const nodeId = (item as HTMLElement).dataset.nodeId || '';
  ...
  contentEl.querySelectorAll('.doc-link-remove').forEach(btn => {
    btn.addEventListener('click', (e: Event) => {
      e.stopPropagation();
      const secIdx = parseInt((btn as HTMLElement).dataset.sectionIdx || '0', 10);
      const linkIdx = parseInt((btn as HTMLElement).dataset.linkIdx || '0', 10);
