...
type MonacoReadyCallback = () => void;

let monacoLoaded = false;
const pendingCallbacks: MonacoReadyCallback[] = [];
 ...
 */
export function ensureMonaco(callback: MonacoReadyCallback): void {
  if (monacoLoaded) {
    callback();
    return;
  }
  ...
  if (pendingCallbacks.length === 1) {
    ...
    req(['vs/editor/editor.main'], () => {
      ...
      for (const cb of pendingCallbacks) {
        try { cb(); } catch (e) { console.error('Monaco callback error:', e); }
      }
...
export function stalenessBadge(status: string): string {
...
export function subtypeBadge(subtype: string): string {
...
export function shortTitle(nodeId: string): string {
  return nodeId.split('::').pop() || nodeId;
}

/** Extract the module/file portion of a node location. */
export function locationModule(location: string | undefined): string {
  if (!location) return '';
