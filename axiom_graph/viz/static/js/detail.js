...
let selectNodeFn = null;
let openSourceWithDiffFn = null;
let onVerifyCompleteFn = null;
let navigateToNodeFn = null;
let lookupNodeFn = null;
export function setDetailCallbacks(selectNode, openSourceWithDiff, onVerifyComplete, navigateToNode, lookupNode) {
...
let stalenessStatus = null;
let dflowStepsData = null;
let testsData = null;
let stalenessCause = null;
// ── Public API ──────────────────────────────────────────────────────────────
export function show(node, history, staleness, dflowSteps, tests, cause) {
    nodeId = node.id;
    nodeData = node;
    historyData = history;
    ...
    if (titleEl)
        titleEl.textContent = node.title || node.id.split('::').pop() || null;
    renderTabs();
...
export function updateStaleness(status, cause) {
...
function wireTabEvents(content) {
    // Copy buttons
    ...
    content.querySelectorAll('.detail-jump-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (!nid)
                return;
            if (navigateToNodeFn)
                navigateToNodeFn(nid);
            else if (selectNodeFn)
                selectNodeFn(nid);
        });
    });
    ...
    content.querySelectorAll('.edge-group-header').forEach(header => {
        header.addEventListener('click', () => {
            const group = header.closest('.edge-group');
            if (!group)
                return;
...
// ── Verify action ───────────────────────────────────────────────────────────
async function doVerify(content) {
    const reasonInput = content.querySelector('.detail-verify-reason');
    ...
    try {
        ...
        if (onVerifyCompleteFn && nid)
            onVerifyCompleteFn(nid, 'VERIFIED');
        // Update graph node if available
        const cy = getCy();
        if (cy) {
            const ele = cy.getElementById(nid);
            if (ele.length)
                ele.data('staleness', 'VERIFIED');
        }
        if (activeTab === 'overview')
            activateTab('overview');
    }
...
function renderOverviewTab() {
    ...
    parts.push(`<div class="detail-section">` +
        ...
        `</div>`);
    // ID
    parts.push(labeledBlock('ID', `<div class="detail-location">${esc(node.id)}` +
    ...
    const locLine = node.level_3_location
        ...
        : esc(node.location);
    parts.push(labeledBlock('Location', `<div class="detail-location">${locLine}` +
        `<button class="copy-btn" data-copy="${esc(node.location)}">copy</button></div>`));
    // Tags
    if (node.tags && node.tags.length > 0) {
        const chips = node.tags.map((t) => `<span class="tag-chip">${esc(t)}</span>`).join('');
        parts.push(labeledBlock('Tags', `<div class="flex flex-wrap gap-4">${chips}</div>`));
    }
    // Summary (level_0)
    if (node.level_0) {
        parts.push(labeledBlock('Summary', `<div class="detail-level-block">${esc(node.level_0)}</div>`));
    }
    // Staleness cause
    if (cause) {
        const isClean = cause.status === 'VERIFIED' || cause.status === 'VERIFIED';
        parts.push(labeledBlock('Staleness Cause', `<div class="detail-cause-block${isClean ? ' clean' : ''}">${esc(cause.cause)}</div>` +
            renderStaleLinkedNodes(cause)));
    }
    // Verification attribution
    if (cause && cause.verification) {
        const v = cause.verification;
        ...
        parts.push(labeledBlock('Verification', `<div class="detail-verification-attr">` +
    ...
    if (staleness && staleness !== 'VERIFIED' && staleness !== 'VERIFIED' && staleness !== 'NOT_FOUND' && staleness !== 'unknown') {
        parts.push(`<div class="detail-verify-form">` +
...
function renderStaleLinkedNodes(cause) {
    if (!cause || !cause.details || !cause.details.stale_linked_nodes)
        return '';
    const nodes = cause.details.stale_linked_nodes;
    if (nodes.length === 0)
        return '';
    const links = nodes.map((nid) => {
...
function renderDocsTab() {
    ...
    const parts = [];
    if (node.level_1) {
        parts.push(labeledBlock('Level 1 \u2014 Description', `<div class="detail-level-block">${esc(node.level_1)}</div>`));
    }
    if (node.level_2) {
        parts.push(labeledBlock('Level 2 \u2014 Details', `<div class="detail-level-block level-2">${esc(node.level_2)}</div>`));
    }
    if (parts.length === 0) {
...
function renderApiTab() {
    ...
    if (node.dflow_meta) {
        ...
        if (autodocHtml)
            parts.push(labeledBlock('Interface', autodocHtml));
    }
    if (node.level_2) {
        parts.push(labeledBlock('Docstring', `<div class="detail-level-block level-2">${esc(node.level_2)}</div>`));
    }
...
function mergeSteps(cortexSteps, dflowData) {
 ...
 */
function compareStepNumbers(a, b) {
    const segments = (v) => String(v ?? '').split('.').map(s => {
...
function renderStepItem(s) {
    ...
    const io = (s.inputs ? `<div class="step-io" style="color:#27ae60">in: ${esc(s.inputs)}</div>` : '') +
    ...
    return (`<div class="step-item${isMinor ? ' step-item-minor' : ''}">` +
...
function renderRelationshipsTab() {
    ...
    if (edges.inbound && edges.inbound.length > 0) {
        parts.push(labeledBlock(`Inbound (${edges.inbound.length})`, renderEdgeGroups(edges.inbound, 'inbound')));
    }
    if (edges.outbound && edges.outbound.length > 0) {
        parts.push(labeledBlock(`Outbound (${edges.outbound.length})`, renderEdgeGroups(edges.outbound, 'outbound')));
    }
    if (parts.length === 0) {
...
function renderEdgeGroups(edges, direction) {
    const groups = {};
    ...
    return sorted.map(([edgeType, edgeList]) => {
        ...
        return (`<div class="edge-group${collapsed}">` +
...
function renderHistoryTab() {
    ...
    if (cause && cause.verification) {
        ...
        parts.push(labeledBlock('Verification Record', `<div class="detail-verification-attr">` +
            `Verified by <strong>${esc(by)}</strong> on ${esc(when)}<br>` +
            `Reason: ${reason}${hash}` +
            `</div>`));
    }
    if (history && history.length > 0) {
        const renames = history.filter((h) => (h.change_type || '').toUpperCase().includes('RENAME'));
        if (renames.length > 0) {
            const rows = renames.map((h) => `<tr>` +
                `<td>${esc((h.scanned_at || '').slice(0, 10))}</td>` +
                `<td>${esc(h.change_type || '')}</td>` +
                `<td style="font-size:10px;color:#999">${esc((h.git_sha || '').slice(0, 8))}</td>` +
                `</tr>`).join('');
            parts.push(labeledBlock('Rename History', `<table class="history-table">` +
    ...
    if (history && history.length > 0) {
        const filtered = history.filter((h) => (h.change_type || '').toUpperCase() !== 'CHECKPOINT');
        ...
        parts.push(labeledBlock(`Node History (${filtered.length})`, `<table class="history-table">` +
...
// ── Autodoc renderer ────────────────────────────────────────────────────────
function renderAutodocInner(meta) {
    const sections = [];
    if (meta.params && meta.params.length > 0) {
        const rows = meta.params.map((p) => {
        ...
        sections.push(`<div class="autodoc-block">` +
    ...
    if (meta.raises_doc && meta.raises_doc.length > 0) {
        const rows = meta.raises_doc.map((r) => {
        ...
        const chips = meta.raises.map((r) => `<span class="autodoc-raises-type">${esc(r)}</span>`).join(' ');
        sections.push(`<div class="autodoc-block">` +
            `<div class="autodoc-heading">Raises</div>` +
            `<div>${chips}</div>` +
            `</div>`);
...
function renderTestCoverage(tests) {
    const statusIcons = {
        'PASS': '\u2705', 'passed': '\u2705', 'pass': '\u2705',
        'FAIL': '\u274c', 'failed': '\u274c', 'fail': '\u274c',
        'ERROR': '\ud83d\udca5', 'error': '\ud83d\udca5',
    ...
    const rows = tests.map(t => {
        ...
        return (`<tr class="test-coverage-row">` +
    ...
    return labeledBlock(`Test Coverage (${tests.length})`, `<table class="test-coverage-table"><tbody>${rows}</tbody></table>`);
}
// ── Helpers ─────────────────────────────────────────────────────────────────
function labeledBlock(label, innerHtml) {
