...
import { getCy, init as graphInit, loadAll as graphLoadAll } from './graph.js';
...
let _monacoDecorations = [];
let _pendingSource = null;
let _currentSourcePath = null;
let _currentSourceData = null;
let _focusMode = false;
let _diffMode = false;
let _diffData = null;
let _diffEditor = null;
// Doc diff state
let _docDiffData = null;
let _docDiffMode = false;
// Doc state
let _docRenderData = null;
let _docFocusSection = null;
let _docShowFull = false;
let _docCurrentId = null;
// ── Source panel ────────────────────────────────────────────────────────────
export async function openSource(nodeId) {
    ...
    if (monacoWrap)
        monacoWrap.classList.remove('hidden');
    if (docWrap)
    ...
    try {
        ...
        if (titleEl)
            titleEl.textContent = data.path || nodeId;
        if (!_monacoReady) {
            if (monacoWrap)
                monacoWrap.innerHTML =
...
export async function openSourceWithDiff(nodeId, sha) {
...
export async function openDocPreview(nodeId) {
    const col = document.getElementById('list-source-col');
    const titleEl = document.getElementById('list-source-title');
    const monacoWrap = document.getElementById('list-monaco-wrap');
    const docWrap = document.getElementById('list-doc-wrap');
    if (col)
        col.classList.remove('hidden');
    if (monacoWrap)
        monacoWrap.classList.add('hidden');
    if (docWrap)
    ...
    try {
        ...
        if (titleEl)
            titleEl.textContent = renderData.title || docId;
        _renderDocPanel();
        _probeDocDiff(docId);
    }
    catch {
        try {
            ...
            _docRenderData = renderData;
            if (titleEl)
                titleEl.textContent = renderData.title || parentDocId;
            _renderDocPanel();
            _probeDocDiff(parentDocId);
        }
        catch (err2) {
            if (titleEl)
                titleEl.textContent = 'Doc preview';
            if (docWrap)
                docWrap.innerHTML = `<div class="list-doc-content"><p style="color:#8b949e">Could not load document: ${esc(err2.message)}</p></div>`;
        }
    }
}
export async function openMarkdownPreview(nodeId) {
    const col = document.getElementById('list-source-col');
    const titleEl = document.getElementById('list-source-title');
    const monacoWrap = document.getElementById('list-monaco-wrap');
    const docWrap = document.getElementById('list-doc-wrap');
    ...
    try {
        ...
        if (docWrap)
            docWrap.innerHTML = `<div class="list-doc-content">${rendered}</div>`;
    }
    catch (err) {
        if (titleEl)
            titleEl.textContent = 'Markdown preview';
        if (docWrap)
            docWrap.innerHTML = `<div class="list-doc-content"><p style="color:#8b949e">Could not load file: ${esc(err.message)}</p></div>`;
    }
    selectNodeFn(nodeId);
}
// ── Doc panel rendering ──────────────────────────────────────────────────────
function _renderDocPanel() {
    ...
    if (!data || !data.sections || !docWrap)
        return;
    const renderMd = (content) => docsRenderMarkdown(content);
    let html = '<div class="list-doc-content">';
    if (_docFocusSection && !_docShowFull) {
        const sec = data.sections.find((s) => s.id === _docFocusSection || s.id.endsWith(_docFocusSection));
        if (sec) {
...
function _renderDocBreadcrumb() {
    ...
    const currentSec = sections.find((s) => s.id === _docFocusSection || s.id.endsWith(_docFocusSection));
    if (!currentSec) {
        bcEl.classList.add('hidden');
        if (upBtn)
            upBtn.classList.add('hidden');
    ...
    for (let i = 0; i < trail.length; i++) {
        if (i > 0)
            html += '<span class="list-bc-sep">\u203a</span>';
    ...
    bcEl.querySelectorAll('.list-bc-link').forEach(link => {
        link.addEventListener('click', () => {
            const secId = link.dataset.bcSectionId;
            if (!secId) {
                _docFocusSection = null;
                _docShowFull = true;
            }
            else {
                _docFocusSection = secId;
                _docShowFull = false;
            }
            if (_docDiffMode)
                _renderDocDiff();
    ...
    const hasChildren = sections.some((s) => s.parent_id === currentSec.id);
    if (upBtn)
        upBtn.classList.toggle('hidden', !hasParent);
    if (downBtn)
        downBtn.classList.toggle('hidden', !hasChildren);
}
// ── Diff probing ─────────────────────────────────────────────────────────────
async function _probeDiff(nodeId) {
    const state = getAppState();
    if (!state.sinceFilter)
        return;
    try {
        let url = `/api/nodes/${encodeURIComponent(nodeId)}/diff`;
        if (state._sinceBaselineSha)
            url += `?sha=${encodeURIComponent(state._sinceBaselineSha)}`;
        const diff = await apiFetch(url);
        if (_currentSourceData && _currentSourceData.node_id !== nodeId)
            return;
        const diffBtn = document.getElementById('list-source-diff-btn');
        if (!diffBtn)
            return;
        ...
        console.warn('[list] _probeDiff failed:', err);
    }
}
async function _probeDocDiff(docId) {
    const state = getAppState();
    if (!state.sinceFilter)
        return;
    try {
        ...
        console.warn('[list] _probeDocDiff failed:', err);
    }
}
// ── Doc diff rendering ───────────────────────────────────────────────────────
function _renderDocDiff() {
    const docWrap = document.getElementById('list-doc-wrap');
...
function _renderMonaco(data) {
    ...
    if (!_monacoEditor) {
        ...
        _monacoEditor = m.editor.create(document.getElementById('list-monaco-container'), {
            value: content, language: lang, readOnly: true, theme: 'vs-dark',
            minimap: { enabled: true }, scrollBeyondLastLine: false, fontSize: 12,
            lineNumbers: _focusMode ? ((ln) => String(ln + (data.focus_start || 1) - 1)) : 'on',
    ...
    if (!_focusMode && data.focus_start)
        _jumpToLine(data.focus_start, data.focus_end);
}
function _jumpToLine(start, end) {
    if (!_monacoEditor || !start)
        return;
    const m = window.monaco;
    _monacoEditor.revealLineInCenter(start);
    _monacoDecorations = _monacoEditor.deltaDecorations(_monacoDecorations, [{ range: new m.Range(start, 1, end || start, 1),
            options: { isWholeLine: true, className: 'wf-monaco-highlight' } }]);
}
function _renderDiffMonaco(diff) {
    const wrap = document.getElementById('list-monaco-wrap');
    if (!wrap)
        return;
    const m = window.monaco;
    ...
    _diffEditor = m.editor.createDiffEditor(document.getElementById('list-monaco-container'), {
    ...
    if (titleEl)
        titleEl.textContent = `Diff: ${sha} (${date}) \u2194 current`;
}
// ── Toggle controls ──────────────────────────────────────────────────────────
export function toggleFocus() {
    ...
    if (_currentSourceData)
        _renderMonaco(_currentSourceData);
}
export function toggleDiff() {
    ...
    if (isDocMode) {
        ...
        const diffBtn = document.getElementById('list-source-diff-btn');
        if (diffBtn)
            diffBtn.classList.toggle('active', _docDiffMode);
        if (_docDiffMode)
            _renderDocDiff();
        else {
            _renderDocPanel();
            const titleEl = document.getElementById('list-source-title');
            if (titleEl && _docRenderData)
                titleEl.textContent = _docRenderData.title || _docCurrentId || 'Doc';
        }
        return;
    }
    if (!_diffData)
    ...
    const diffBtn = document.getElementById('list-source-diff-btn');
    if (diffBtn)
        diffBtn.classList.toggle('active', _diffMode);
    if (_diffMode) {
...
export function viewInGraph() {
    ...
    const subNodes = state.allNodes.filter((n) => nodeSet.has(n.id));
    const subEdges = state.allEdges.filter((e) => nodeSet.has(e.from_id) && nodeSet.has(e.to_id));
    if (!getCy())
        graphInit(document.getElementById('cy'));
...
export function getDocCurrentId() { return _docCurrentId; }
// ── Navigation helpers ───────────────────────────────────────────────────────
function _navigateDocUp() {
    const data = _docRenderData;
    if (!data || !data.sections || !_docFocusSection)
        return;
    const currentSec = data.sections.find((s) => s.id === _docFocusSection || s.id.endsWith(_docFocusSection));
    if (!currentSec || !currentSec.parent_id)
        return;
...
function _navigateDocDown() {
    ...
    if (!currentSec)
        return;
    const firstChild = data.sections.find((s) => s.parent_id === currentSec.id);
    if (!firstChild)
        return;
...
// ── VSCode URI helper ────────────────────────────────────────────────────────
export function vscodeUri(location) {
...
export function bindSourcePanelHandlers() {
    const focusBtn = document.getElementById('list-source-focus-btn');
    if (focusBtn)
    ...
    if (navUpBtn)
        navUpBtn.addEventListener('click', () => _navigateDocUp());
    const navDownBtn = document.getElementById('list-doc-nav-down');
    if (navDownBtn)
        navDownBtn.addEventListener('click', () => _navigateDocDown());
}
// ── Monaco init ─────────────────────────────────────────────────────────────
