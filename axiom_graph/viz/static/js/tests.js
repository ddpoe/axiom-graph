...
let colResizeInit = () => { };
export function setColResizeInit(fn) {
    colResizeInit = fn;
}
// ── State ───────────────────────────────────────────────────────────────────
let _tests = [];
...
async function fetchTests() {
    ...
    try {
        ...
        if (container)
            container.innerHTML = `<div class="wf-status wf-error">Failed to load tests: ${esc(err.message)}</div>`;
    }
}
function tier(t) { return t.tier || 'T1'; }
function hasTierDiversity() { return _tests.some(t => t.tier !== 'T1'); }
async function refreshTests() {
    if (_refreshInProgress)
        return;
    _refreshInProgress = true;
    const btn = document.getElementById('tests-refresh-btn');
    if (btn) {
        btn.classList.add('tests-refresh-spinning');
        btn.disabled = true;
    }
    try {
        ...
        console.error('Tests refresh failed:', err);
    }
    finally {
        _refreshInProgress = false;
        if (btn) {
            btn.classList.remove('tests-refresh-spinning');
            btn.disabled = false;
        }
    }
}
function applyFilters(items) {
    return items.filter(w => {
        if (_fHideT1 && w.is_t1)
            return false;
        if (_fHasSteps && w.step_count === 0)
            return false;
        if (_fHasCritical && !w.critical)
            return false;
        if (_fIsSkeleton && w.critical !== 'NOT IMPLEMENTED')
            return false;
        if (_fHasCovers && !(w.validates_count > 0))
            return false;
        if (_fLinked && !w.cortex_node_id)
            return false;
        if (_modSelected.size > 0 && !_modSelected.has(w.module))
            return false;
        return true;
    });
}
function render() {
    ...
    if (refreshBtn)
        refreshBtn.addEventListener('click', () => refreshTests());
...
function buildFilterPanel(panel) {
    ...
    for (const t of _tests)
        modCount.set(t.module, (modCount.get(t.module) || 0) + 1);
    ...
    const quickHtml = quickFilters.map(f => `<label class="wf-fp-check"><input type="checkbox" data-key="${f.key}"${f.checked ? ' checked' : ''}>${f.label}</label>`).join('');
    ...
    panel.querySelectorAll('input[data-key]').forEach(cb => {
        cb.addEventListener('change', () => {
            ...
            if (key === '_fHideT1')
                _fHideT1 = checked;
            else if (key === '_fHasSteps')
                _fHasSteps = checked;
            else if (key === '_fHasCritical')
                _fHasCritical = checked;
            else if (key === '_fIsSkeleton')
                _fIsSkeleton = checked;
            else if (key === '_fHasCovers')
                _fHasCovers = checked;
            else if (key === '_fLinked')
                _fLinked = checked;
            updateList();
    ...
    const modSearchEl = panel.querySelector('.wf-fp-mod-search');
    if (modSearchEl)
        modSearchEl.addEventListener('input', () => { _modSearch = modSearchEl.value; buildFilterPanel(panel); });
    panel.querySelectorAll('.wf-fp-mod-list').forEach(list => {
        list.addEventListener('change', e => {
            ...
            if (_modSelected.size === 0)
                modules.forEach(([p]) => _modSelected.add(p));
            if (cb.checked)
                _modSelected.add(path);
            else
                _modSelected.delete(path);
            if (_modSelected.size === modules.length)
                _modSelected.clear();
            updateList();
            _refreshClearBtn(panel);
        });
    ...
    if (clearBtn)
        clearBtn.addEventListener('click', () => _clearFilters(panel));
}
function _clearFilters(panel) {
    _fHideT1 = _fHasSteps = _fHasCritical = _fIsSkeleton = _fHasCovers = _fLinked = false;
    _modSelected.clear();
    buildFilterPanel(panel);
    updateList();
}
function _refreshClearBtn(panel) {
...
function updateList() {
    ...
    if (_groupBy === 'module') {
        ...
        for (const [modPath, groupItems] of groups) {
            ...
            if (!collapsed) {
                for (const t of groupItems)
                    listEl.appendChild(makeListItem(t));
            }
        }
    }
    else if (_groupBy === 'tier') {
        const tierOrder = ['T3', 'T2', 'T1'];
        const tierLabel = {
            T3: 'T3 \u2014 Workflow + steps',
            T2: 'T2 \u2014 Workflow entry',
            T1: 'T1 \u2014 Unannotated',
        };
        const groups = new Map(tierOrder.map(t => [t, []]));
        for (const t of filtered) {
            const k = tier(t);
            if (!groups.has(k))
                groups.set(k, []);
        ...
        for (const t of tierOrder) {
            ...
            const paths = [...new Set(groupItems.map(w => w.module))];
            ...
            if (!collapsed) {
                for (const w of groupItems)
                    listEl.appendChild(makeListItem(w));
            }
        }
    }
    else {
...
async function selectTest(t) {
    ...
    document.querySelectorAll('.wf-list-item').forEach(el => {
        el.classList.toggle('active', el.dataset.testId === String(t.id));
    });
    ...
    if (t.is_t1) {
        ...
        if (t.module)
            openSource(t.module, t.line_start || 1);
        return;
    }
    ...
    try {
        ...
        if (data.module)
            openSource(data.module, data.line_start || 1);
    }
    catch {
...
function renderTestDetail(stepsCol, data, t) {
    ...
    if (func.purpose)
        html += `<div class="wf-detail-purpose">${esc(func.purpose)}</div>`;
    const funcMeta = [];
    if (func.inputs)
        funcMeta.push(`<span class="wf-meta-chip">in: ${esc(func.inputs)}</span>`);
    if (func.outputs)
        funcMeta.push(`<span class="wf-meta-chip">out: ${esc(func.outputs)}</span>`);
    if (funcMeta.length)
        html += `<div class="wf-func-meta">${funcMeta.join('')}</div>`;
    if (func.critical)
        html += `<div class="wf-step-critical${func.critical === 'NOT IMPLEMENTED' ? '--skeleton' : ''}">&#9888; ${esc(func.critical)}</div>`;
    html += '</div>';
    // Steps
    if (steps.length > 0) {
        ...
        for (const step of steps) {
            ...
            if (step.outputs)
                stepMeta.push(`out: ${esc(step.outputs)}`);
            if (stepMeta.length)
                html += `<div class="wf-step-meta">${stepMeta.join(' &middot; ')}</div>`;
            if (step.critical)
                html += `<div class="wf-step-critical${step.critical === 'NOT IMPLEMENTED' ? '--skeleton' : ''}">&#9888; ${esc(step.critical)}</div>`;
    ...
    if (fixtures.length > 0) {
        ...
        for (const fix of fixtures) {
            ...
            if (locText)
                html += `<div class="wf-fixture-loc">${locText}</div>`;
            if (fix.docstring)
                html += `<div class="wf-fixture-doc">${esc(fix.docstring)}</div>`;
            const actions = [];
            if (fix.location) {
            ...
            if (actions.length)
                html += `<div class="wf-fixture-actions">${actions.join('')}</div>`;
            html += '</li>';
        }
        html += '</ul></div>';
    }
        ...
        for (const fid of t.fixture_ids) {
            ...
            if (f) {
                ...
                if (f.location)
                    html += `<div class="wf-fixture-loc">${esc(f.location)}</div>`;
                if (f.docstring)
                    html += `<div class="wf-fixture-doc">${esc(f.docstring)}</div>`;
                html += '</div>';
            }
        }
        html += '</div>';
    ...
    stepsCol.querySelectorAll('.wf-step-link[data-source-path]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            const line = parseInt(btn.dataset.sourceLine || '1', 10);
            if (path)
    ...
    stepsCol.querySelectorAll('.wf-step[data-line]').forEach(li => {
        ...
        if (line)
            li.addEventListener('click', () => jumpToLine(line));
    });
}
function renderSimpleDetail(stepsCol, t) {
    const tierVal = tier(t);
    const tierCls = tierVal === 'T3' ? 'tier3' : tierVal === 'T2' ? 'tier2' : 'tier1';
    let html = `<div class="wf-detail-inner">
...
function jumpToLine(line) {
    if (!_monacoEditor || !line)
        return;
    const m = window.monaco;
    _monacoEditor.revealLineInCenter(line);
    _monacoDecorations = _monacoEditor.deltaDecorations(_monacoDecorations, [{ range: new m.Range(line, 1, line, 1), options: { isWholeLine: true, className: 'wf-monaco-highlight' } }]);
}
...
function _makeGroupHeader(label, groupKey, groupItems, collapsed = false) {
...
function _toggleCollapseAll() {
    ...
    if (_allCollapsed) {
        ...
        if (_groupBy === 'module') {
            for (const t of filtered)
                _collapsedGroups.add(t.module || '');
        }
        else if (_groupBy === 'tier') {
            for (const t of filtered)
                _collapsedGroups.add(tier(t));
        }
    }
    else {
        _collapsedGroups.clear();
