...
let _callbacks = null;
let _allCommits = [];
let _checkedShas = []; // 0, 1, or 2 SHAs
let _expandedShas = new Set();
let _searchTerm = '';
let _dateFilter = '';
...
export function initSinceModal(callbacks) {
    ...
    const searchInput = document.getElementById('since-modal-search');
    if (searchInput) {
    ...
    const dateInput = document.getElementById('since-modal-date');
    if (dateInput) {
...
export async function openCommitPicker() {
    const overlay = _overlay();
    if (!overlay)
        return;
    // Reset state
...
function _createCommitRow(commit, rangeSet) {
...
function _getRangeSet(commits) {
    const result = new Set();
    if (_checkedShas.length !== 2)
        return result;
    const indices = _checkedShas.map(sha => commits.findIndex(c => c.sha === sha)).filter(i => i >= 0).sort((a, b) => a - b);
    if (indices.length !== 2)
        return result;
...
function _applyRange() {
    if (!_callbacks || _checkedShas.length !== 2)
        return;
...
function _formatDateSeparator(dateStr) {
    try {
...
function _updateHint(count) {
    const hint = document.querySelector('.since-modal-hint');
    if (!hint)
        return;
    if (_searchTerm || _dateFilter) {
        hint.textContent = `${count} commit${count !== 1 ? 's' : ''} matching. Click a row = filter since that commit. Check two = select a range. Esc to close.`;
    }
    else {
        hint.textContent = 'Click a row = filter since that commit. Check two = select a range. Esc to close.';
    }
}
