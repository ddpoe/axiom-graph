...
export async function init() {
    if (!_initialized) {
        ...
        Detail.setDetailCallbacks(selectNode, List.openSourceWithDiff, (nid, status) => {
            state.stalenessMap[nid] = { own_status: status, link_status: 'VERIFIED' };
            _renderStalenessSummary(state.stalenessMap);
        }, navigateToNode, (nid) => {
            if (!state.allNodes)
                return null;
            const n = state.allNodes.find((node) => node.id === nid);
        ...
        List.setListCallbacks({
            ...
            colResizeWire: (handle) => ColResize._wire(handle),
        });
        ...
        document.querySelectorAll('input[name="group-by"]').forEach(radio => {
            radio.addEventListener('change', e => {
                List.setGroupBy(e.target.value || null);
            });
        });
        ...
        document.getElementById('show-tests-check').addEventListener('change', e => {
            state.filters.showTests = e.target.checked;
            _applyFiltersAndRefresh();
        });
        ...
        document.querySelectorAll('.layout-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                state.layout = btn.dataset.layout || 'cose';
        ...
        document.getElementById('search-input').addEventListener('input', e => {
            ...
            searchTimer = setTimeout(() => onSearch(e.target.value), 300);
        });
        ...
        document.querySelectorAll('.since-preset').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                applySinceFilter(btn.dataset.since || '');
            });
        });
...
async function _loadProject() {
    ...
    try {
        ...
        for (const et of meta.edge_types)
            state.filters.edgeTypes[et] = true;
        _populateEdgeTypeFilters(meta.edge_types);
        state.selectedNodeId = null;
        state.stepEdgeLabels = null;
        _applyFilters();
        if (state.isLargeProject) {
            _showGraphNotice(`Large project \u2014 ${meta.node_count} nodes detected. Using list view by default.`, () => _loadFullGraph());
            setView('list');
        }
        else {
...
function renderDocsView() {
    ...
    if (filterPanel)
        DocTree.buildFilterPanel(filterPanel, () => renderDocsView());
    // Render folder tree
    const listEl = document.getElementById('doc-list');
    if (listEl) {
        DocTree.renderFolderTree(listEl, filtered, docsSelectDoc, DocApi.getSelectedDocId(), async (docId) => {
            ...
            // tell them apart.
            const stripPrimary = (d) => {
                const clean = primary.replace(/\/+$/, '');
                if (d === clean)
                    return '';
                if (clean && d.startsWith(clean + '/'))
                    return d.slice(clean.length + 1);
                return null;
            };
          ...
          <div class="doc-move-dialog">
            ...
            overlay.addEventListener('click', (e) => { if (e.target === overlay)
                cleanup(); });
            const sel = overlay.querySelector('.doc-move-select');
...
function _scrollDocIntoView(docId) {
...
function _renderSelectedDoc(contentArea) {
    DocEditor.renderDocContent(contentArea);
}
function _flattenRawSections(sections) {
    const result = [];
    for (const sec of sections) {
        const { sections: children, ...rest } = sec;
        result.push(rest);
        if (children && children.length)
            result.push(..._flattenRawSections(children));
    }
    return result;
}
// ── Sidebar population ──────────────────────────────────────────────────────
function _populateSidebarFilters(meta) {
...
function _renderTagFilterPanel() {
    ...
    const byCount = [..._allTags].sort((a, b) => b[1] - a[1]);
    const unselected = byCount.filter(([t]) => !fTags.has(t));
    const matching = searchLower ? unselected.filter(([t]) => t.toLowerCase().includes(searchLower)) : unselected;
    const visible = matching.slice(0, _TAG_LIMIT);
    const overflow = matching.length - visible.length;
    const suggestions = visible.map(([t, count]) => `<div class="tag-fp-suggestion" data-tag="${escHtml(t)}"><span class="tag-fp-sug-name">${escHtml(t)}</span><span class="tag-fp-sug-count">${count}</span></div>`).join('');
    panel.innerHTML =
    ...
    panel.querySelectorAll('.tag-fp-chip').forEach(el => {
        el.addEventListener('click', () => { fTags.delete(el.dataset.tag || ''); _renderTagFilterPanel(); _applyFiltersAndRefresh(); });
    });
    panel.querySelectorAll('.tag-fp-suggestion').forEach(el => {
        el.addEventListener('click', () => { fTags.add(el.dataset.tag || ''); state.tagSearch = ''; _renderTagFilterPanel(); _applyFiltersAndRefresh(); });
    });
    const searchInput = panel.querySelector('.tag-fp-search');
    if (searchInput)
        searchInput.addEventListener('input', e => { state.tagSearch = e.target.value; _renderTagFilterPanel(); });
    const showAll = panel.querySelector('.tag-fp-show-all');
    if (showAll)
        showAll.addEventListener('click', () => { _TAG_LIMIT = 999; _renderTagFilterPanel(); });
    const clearBtn = panel.querySelector('.tag-fp-clear');
    if (clearBtn)
        clearBtn.addEventListener('click', () => { fTags.clear(); state.tagSearch = ''; _TAG_LIMIT = 8; _renderTagFilterPanel(); _applyFiltersAndRefresh(); });
}
function _renderStalenessSummary(stalenessMap) {
    const el = document.getElementById('staleness-summary');
    if (!el)
        return;
    const catMap = {};
    for (const n of state.allNodes)
        catMap[n.id] = _nodeCategory(n);
    const byCat = {};
    const totals = {};
    for (const [nodeId, entry] of Object.entries(stalenessMap)) {
        const status = displayStaleness(entry);
        if (status === 'VERIFIED' || status === 'VERIFIED' || status === 'unknown')
            continue;
    ...
    if (Object.keys(totals).length === 0) {
        el.innerHTML = '<span class="staleness-all-clean">\u2713 All clean</span>';
        return;
    }
    el.innerHTML = '';
    for (const [status, total] of Object.entries(totals)) {
        ...
        chip.addEventListener('click', () => {
            const radio = document.querySelector(`input[name="staleness-radio"][value="${status}"]`);
            if (radio) {
                radio.checked = true;
...
function _populateStalenessFilter() {
    const values = [
...
function _populateEdgeTypeFilters(edgeTypes) {
...
function _populateSubtypeFilters(nodes) {
    const counts = {};
...
function _renderSidebarStats(meta) {
    const el = document.getElementById('sidebar-stats');
    if (el)
        el.innerHTML = `<div><strong>${meta.node_count}</strong> nodes</div><div><strong>${meta.edge_count}</strong> edges</div><div><strong>${meta.edge_types.length}</strong> edge types</div><div><strong>${meta.tags.length}</strong> tags</div>`;
}
// ── Filter event handlers ───────────────────────────────────────────────────
function _onTypeFilterChange(e) {
...
function _onSubtypeFilterChange(e) {
    const st = e.target.dataset.subtype || '';
    if (e.target.checked)
        state.filters.subtypes.delete(st);
    else
        state.filters.subtypes.add(st);
    _applyFiltersAndRefresh();
}
function _onEdgeTypeFilterChange(e) {
    const et = e.target.dataset.edgeType || '';
    state.filters.edgeTypes[et] = e.target.checked;
    Graph.applyEdgeFilter(state.filters.edgeTypes);
    if (state.filters.hideOrphans)
        Graph.hideOrphanNodes(new Set(state.filteredNodes.map(n => n.id)));
}
function _onHideOrphansChange(e) {
    state.filters.hideOrphans = e.target.checked;
    const baseIds = new Set(state.filteredNodes.map(n => n.id));
    if (state.filters.hideOrphans)
        Graph.hideOrphanNodes(baseIds);
    else
        Graph.filterNodes(baseIds);
}
function _onStalenessFilterChange(e) {
    state.filters.staleness = e.target.value;
    _applyFiltersAndRefresh();
}
// ── Filter logic ────────────────────────────────────────────────────────────
function _applyFilters() {
    ...
    if (state.filters.types.size > 0)
        nodes = nodes.filter(n => !state.filters.types.has(n.node_type));
    if (state.filters.subtypes.size > 0)
        nodes = nodes.filter(n => { const st = n.subtype || (n.node_type === 'composite_process' ? 'module' : 'function'); return !state.filters.subtypes.has(st); });
    if (state.filters.tags.size > 0)
        nodes = nodes.filter(n => { const tagSet = new Set(n.tags || []); for (const t of state.filters.tags) {
            if (!tagSet.has(t))
                return false;
        } return true; });
    if (state.filters.staleness !== 'all') {
        const target = state.filters.staleness;
        nodes = nodes.filter(n => displayStaleness(state.stalenessMap[n.id]) === target);
    }
    if (!state.filters.showPrivate)
        nodes = nodes.filter(n => { const name = (n.title || '').split('.').pop() || ''; return !name.startsWith('_'); });
    if (!state.filters.showTests && state.meta && state.meta.test_paths) {
        const testPaths = state.meta.test_paths;
...
function _applyFiltersAndRefresh() {
    ...
    if (state.view === 'list')
        ...
        if (state._sinceNodeIds) {
            ...
            Graph.applyEdgeFilter(state.filters.edgeTypes);
            if (state.filters.hideOrphans && state.filters.staleness === 'all')
                Graph.hideOrphanNodes(visibleIds);
        }
    }
}
// ── Since filter ────────────────────────────────────────────────────────────
 ...
 */
function _renderSinceBanner(opts) {
...
async function applySinceFilter(type, customValue, opts) {
    ...
    if (type === 'checkpoint') { /* no params — default resolution */ }
        ...
        try {
            const data = await apiFetch('/api/history/recent-shas');
        ...
        params.set('timestamp', new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString());
    else if (type === '7d')
        params.set('timestamp', new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString());
    else if (type === 'custom' && customValue) {
        if (/^[0-9a-f]{6,40}$/i.test(customValue))
            params.set('sha', customValue);
        else
            params.set('timestamp', customValue);
    }
    setStatus('Filtering\u2026');
    try {
        ...
        if (delCount > 0)
            statusParts.push(`${delCount} deleted`);
        setStatus(`Since filter: ${statusParts.join(', ')}`);
    }
    catch (err) {
        setStatus(`Since filter error: ${err.message}`);
    }
}
async function applySinceRange(sinceSha, untilSha, sinceSubject, untilSubject) {
    ...
    try {
        ...
        setStatus(`Range filter error: ${err.message}`);
    }
}
function clearSinceFilter() {
...
function _updateSinceUI(activeType) {
    // Highlight active preset button
    document.querySelectorAll('.since-preset').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.since === activeType);
    });
    // Toggle clear button
...
function _renderSinceSummary() {
    ...
    return (`<div class="since-summary-type">${typeLabel}</div>` +
        shaHtml +
        subjectHtml +
        `<div class="since-summary-count">${countText}</div>` +
        `<button class="since-summary-clear">&times; Clear</button>`);
...
export async function selectNode(nodeId) {
    ...
    try {
        ...
        if (matchingWf)
            basePromises.push(apiFetch(`/api/workflow/${matchingWf.id}/steps`));
...
function navigateToNode(targetNodeId) {
...
function _renderDetailActions(nodeId) {
    const el = document.getElementById('detail-actions');
    if (!el)
        return;
    el.innerHTML = '';
...
function _expandFromNode(nodeId) {
    ...
    Graph.applyEdgeFilter(state.filters.edgeTypes);
    if (state.filters.hideOrphans)
        Graph.hideOrphanNodes(newVisible);
    setStatus(`Expanded: +${toAdd.length} node${toAdd.length === 1 ? '' : 's'} \u00b7 ${newVisible.size} total visible`);
}
function _bfsNeighborhood(seedId, maxDepth, direction) {
    const out = {}, inc = {};
    for (const e of state.allEdges) {
        (out[e.from_id] = out[e.from_id] || []).push(e.to_id);
        (inc[e.to_id] = inc[e.to_id] || []).push(e.from_id);
    }
    ...
    for (let d = 0; d < maxDepth; d++) {
        ...
        for (const nid of frontier) {
            const neighbors = [];
            if (direction !== 'in')
                neighbors.push(...(out[nid] || []));
            if (direction !== 'out')
                neighbors.push(...(inc[nid] || []));
...
async function check() {
    const headerBtn = document.getElementById('btn-check');
    const listBtn = document.getElementById('btn-list-check');
    headerBtn.textContent = '\u2b21 Checking\u2026';
    headerBtn.classList.add('loading');
    headerBtn.disabled = true;
    if (listBtn) {
        listBtn.disabled = true;
        listBtn.textContent = '\u2b21 Checking\u2026';
    }
    try {
        ...
        if (Graph.getCy())
            Object.entries(data.statuses).forEach(([id, entry]) => { const ele = Graph.getCy().getElementById(id); if (ele.length)
                ele.data('staleness', displayStaleness(entry)); });
        if (state.view === 'list')
            List.render(state.filteredNodes, state.stalenessMap);
        _renderStalenessSummary(state.stalenessMap);
        const s = data.summary || {};
        const parts = Object.entries(s).filter(([, n]) => n > 0).map(([k, n]) => `${n} ${k}`);
        ...
        headerBtn.textContent = '\u2b21 Check';
        headerBtn.classList.remove('loading');
        headerBtn.disabled = false;
        if (listBtn) {
            listBtn.disabled = false;
            listBtn.textContent = '\u2b21 Run Check';
        }
    }
}
async function rescan() {
    ...
    try {
        ...
        setStatus('Rescan complete \u2014 reloading\u2026');
...
async function _loadDflowData() {
    try {
        ...
        if (labelData.available && labelData.labels.length > 0) {
            const map = {};
            for (const { from_cortex_node_id, to_cortex_node_id, step_number } of labelData.labels)
                map[`${from_cortex_node_id}|${to_cortex_node_id}`] = `Step ${step_number}`;
            state.stepEdgeLabels = map;
            if (Graph.getCy() && !state.isLargeProject)
                Graph.loadAll(state.filteredNodes, state.allEdges, state.stalenessMap, state.layout, state.stepEdgeLabels);
        }
        if (wfData.available)
            state.dflowWorkflows = wfData.workflows || [];
    }
    catch (err) {
        console.warn('Workflow graph data unavailable:', err.message);
    }
}
function _initProjectSwitcher() {
            ...
            for (const p of data.projects) {
                ...
                if (!p.active)
                    li.addEventListener('click', async () => {
                        ...
                        try {
                            await fetch('/api/projects/switch', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ project_root: p.path }) }).then(r => { if (!r.ok)
    ...
    document.addEventListener('click', e => { if (!e.target.closest('#project-switcher'))
        dropdown.classList.add('hidden'); });
    ...
    const register = async () => {
        ...
        try {
            const res = await fetch('/api/projects/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ project_root: path }) });
            if (!res.ok) {
...
export const ColResize = {
    init(container) {
        container.querySelectorAll('.col-resize-handle').forEach(h => ColResize._wire(h));
    },
    _wire(handle) {
        handle.addEventListener('mousedown', e => {
            const useNext = handle.dataset.resizeTarget === 'next';
            const targetEl = (useNext ? handle.nextElementSibling : handle.previousElementSibling);
            if (!targetEl)
                return;
            ...
            function onMove(e) { const dx = useNext ? (startX - e.clientX) : (e.clientX - startX); const newWidth = Math.max(100, startWidth + dx); targetEl.style.width = newWidth + 'px'; targetEl.style.flex = 'none'; }
