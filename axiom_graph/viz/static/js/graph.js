    ...
    return [
        {
            selector: 'node',
            style: {
                'background-color': (ele) => SUBTYPE_COLORS[ele.data('subtype')] || NODE_TYPE_COLORS[ele.data('node_type')] || NODE_TYPE_DEFAULT,
                'label': 'data(title)',
                'font-size': '10px',
                'text-valign': 'bottom',
                'text-halign': 'center',
        ...
        { selector: 'node.dimmed', style: { opacity: 0.15 } },
        {
            selector: 'edge',
            style: {
                'line-color': (ele) => EDGE_TYPE_COLORS[ele.data('edge_type')] || EDGE_DEFAULT,
                'target-arrow-color': (ele) => EDGE_TYPE_COLORS[ele.data('edge_type')] || EDGE_DEFAULT,
...
// ── Layouts ──────────────────────────────────────────────────────────────────
function layoutOptions(name, callback) {
    const opts = {
        cose: {
            name: 'cose',
            animate: false,
...
// ── Element builders ────────────────────────────────────────────────────────
function nodeEls(nodes, stalenessMap) {
    return nodes.map(n => ({
        data: {
            id: n.id,
...
function edgeEls(edges, stepLabels) {
    return edges.map(e => {
...
let onNodeTap = null;
let onBackgroundTap = null;
export function setEventHandlers(selectNode, closeDetail) {
...
// ── Load helpers ────────────────────────────────────────────────────────────
export function loadAll(nodes, edges, stalenessMap, layoutName = 'cose', stepLabels = null) {
    if (!cy)
        return;
    ...
    cy.nodes().forEach((n) => {
        if ((inDegree[n.data('id')] || 0) > 3)
            n.addClass('hub');
    });
    cy.layout(layoutOptions(layoutName)).run();
}
export function loadNeighborhood(nodes, edges, centerId, stalenessMap, layoutName = 'cose', stepLabels = null) {
    if (!cy)
        return;
    cy.elements().remove();
    cy.add(nodeEls(nodes, stalenessMap));
...
export function filterNodes(visibleIds) {
    if (!cy)
        return;
    cy.nodes().forEach((n) => {
...
export function highlightNodes(matchIds) {
    if (!cy)
        return;
    cy.nodes().forEach((n) => {
        if (matchIds.has(n.data('id')))
...
export function applyEdgeFilter(edgeTypes) {
    if (!cy)
        return;
    cy.edges().forEach((e) => {
...
export function hideOrphanNodes(baseVisibleIds) {
    if (!cy)
        return;
    cy.nodes().forEach((n) => {
        const nid = n.data('id');
        if (!baseVisibleIds.has(nid))
            return;
        const hasVisibleEdge = n.connectedEdges().some((e) => e.style('display') !== 'none' && !e.hasClass('hidden-type'));
