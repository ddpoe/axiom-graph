...
export async function load() {
    ...
    if (savedId && !_selectedId)
        _selectedId = parseInt(savedId, 10) || savedId;
    await fetchCurrent();
}
// ── Internal ────────────────────────────────────────────────────────────────
...
async function refreshWorkflows() {
    ...
    const btn = document.getElementById('wf-refresh-btn');
    if (btn) {
...
function buildFilterPanel(panel) {
    ...
    for (const w of items)
        modCount.set(w.module, (modCount.get(w.module) || 0) + 1);
    ...
    panel.querySelectorAll('input[name="wf-group"]').forEach(rb => {
        rb.addEventListener('change', () => {
            _groupByMod = rb.value === 'module';
            _collapsedGroups.clear();
            _allCollapsed = true;
            updateList();
        });
    });
    const modSearch = panel.querySelector('#wf-mod-search');
    if (modSearch) {
...
function attachModListEvents(panel, modules) {
...
function rebuildModList(panel, modules) {
    const modList = panel.querySelector('#wf-mod-list');
    if (!modList)
        return;
    const filteredMods = modules.filter(([path]) => !_modSearch || modDisplayName(path).toLowerCase().includes(_modSearch.toLowerCase()));
...
function updateList() {
    ...
    if (_groupByMod) {
        ...
        for (const [modPath, groupItems] of groups) {
            ...
            if (!collapsed) {
                for (const wf of groupItems)
                    listEl.appendChild(makeListItem(wf));
            }
        }
    }
    else {
    ...
    if (toSelect)
        selectWorkflow(toSelect);
}
function makeListItem(wf) {
    ...
    if (wf.cortex_node_id)
        badges.push('<span class="wf-linked" title="Linked to cortex node">&#10003;</span>');
...
async function selectWorkflow(wf) {
    ...
    document.querySelectorAll('.wf-list-item').forEach(el => {
        el.classList.toggle('active', el.dataset.wfId === String(wf.id));
    });
...
function renderDetail(stepsCol, data, wf) {
    ...
    if (wf.module)
        html += `<div class="wf-detail-module">${esc(wf.module)}</div>`;
    if (func.purpose)
    ...
    if (func.critical)
        html += `<div class="wf-critical">&#9888; ${esc(func.critical)}</div>`;
    if (wf.cortex_node_id) {
    ...
    if (steps.length === 0) {
        ...
        for (const step of steps) {
            ...
            if (step.critical)
                html += `<div class="wf-step-critical">&#9888; ${esc(step.critical)}</div>`;
    ...
    stepsCol.querySelectorAll('.wf-step-source-link').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (path)
                openSource(path, line || 1);
        });
    });
}
async function openSource(path, line, colId = 'wf-source-col') {
