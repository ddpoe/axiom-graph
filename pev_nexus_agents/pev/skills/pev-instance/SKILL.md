...
If a path above doesn't resolve, the SOP is very likely still in the graph: when `.pev` is a configured `docs_dirs` root, `.pev/test-policy.json` is indexed as `{project_id}::docs.test-policy` — every root flattens into the one `docs.` namespace, so `.pev` never shows up in a doc id. Reach it with `axiom_graph_read_doc`, or `axiom_graph_search("test policy")`. Never conclude a root is unindexed just because no listing mentions it.
...
- [ ] **Link audit done?** Confirm Step 6 item 2 ran in *both* directions: term families searched for missing edges (**add**), and the changed nodes' existing inbound edges walked for wrong-granularity (**repoint**) or noise (**drop**). Every add/repoint/drop went through the Step 6 item 5 human gate (verb + existing links shown, applied inline only on approval) — never silently changed.
- [ ] **Workflow-marker check?** If the change touched a function that appears in `axiom_graph_workflow_list(steps=true)`, verify the step markers still match the code behavior. Update them if needed (Builder responsibility, unlike full `/pev-cycle` where markers are Reviewer's to flag). When you write or update markers, use the `axiom-annotations-markers` skill for the syntax and numbering rules.
